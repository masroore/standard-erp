...
<head>
    ...
    <title>  {{ $lang == 'ar' ? 'تسجيل الدخول الي النظام' :  'Login To System' }}</title>
    ...
    <link href="{{asset('public/backend/crock/rtl/assets/css/authentication/form-2.css')}}" rel="stylesheet" type="text/css" />
    ...
    <link href="{{asset('public/backend/crock/assets/css/authentication/form-2.css')}}" rel="stylesheet" type="text/css" />
    <!-- END GLOBAL MANDATORY STYLES -->
...
<body class="form">
            ...
            <div class="form-form-wrap">
                <div class="form-container">
                    <div class="form-content">
                        ...
                        <h1 class=""> {{ $lang == 'ar' ? 'تسجيل الدخول  ' :  'Sign In' }}</h1>
                        <p class="">  {{ $lang == 'ar' ? 'تسجيل الدخول إلى حسابك للمتابعة' :  'Log in to your account to continue.' }}</p>
                        ...
                        <form class="text-left" method="POST" action="{{ route('login') }}">
                            ...
                            <div class="form">
                                ...
                                <div id="email-field" class="field-wrapper input">
                                    <label for="email">{{ $lang == 'ar' ? 'البريد الالكتروني ' :  'Email' }}</label>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-user"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                                    <input id="email" name="email" type="email" class="form-control @error('email') is-invalid @enderror" placeholder="examble@domain.com" required>
                                    @error('email')
                                ...
                                <div id="password-field" class="field-wrapper input mb-2">
                                    <div class="d-flex justify-content-between">
                                        <label for="password"> {{ $lang == 'ar' ? 'كلمة المرور  ' :  'PASSWORD' }}</label>
                                        <a href="{{ route('password.request') }}" class="forgot-pass-link"> {{ $lang == 'ar' ? 'نسيت كلمة المرور ؟' :  'Forgot Password?' }}</a>
                                    </div>
                                ...
                                <div class="d-sm-flex justify-content-between">
                                    <div class="field-wrapper">
                                        <button type="submit" class="btn btn-primary" value=""> {{ $lang == 'ar' ? ' تسجيل الدخول ' :  'Log In' }}</button>
                                    </div>
                                </div>
    ...
    <script src="{{asset('public/backend/crock/assets/js/authentication/form-2.js')}}"></script>
