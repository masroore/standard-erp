        ...
        <div class="invoice-detail-title">
            <h4 class="font-weight-bold"> @lang('site.edit_os_customers')</h4>
            <a href="{{ route('dashboard.customer-order-supply.index') }}" class="btn btn-primary float-right"><i class="fa fa-list"></i> @lang('site.all_os_customers')</a>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control basic" name="customer_id">
                        ...
                        @foreach ($customers as $customer)
                            <option value="{{ $customer->id }}" {{ $row->customer_id == $customer->id ? 'selected' : ''}}>{{ $customer->company_name }} ({{ $customer->name }})</option>
                        @endforeach
