...
@section('title')
       @lang('site.purchases_invoices_payments')
@endsection
 @section('modelTitlie')
 @lang('site.purchases_invoices_payments')
 @endsection
...
@component('backend.partials._pagebar')
    ...
    <li class="breadcrumb-item"><a href="{{ route('dashboard.purchases.index') }}">  @lang('site.purchases_list')</a></li>
    <li class="breadcrumb-item active" aria-current="page"><span>@lang('site.payment_list')</span></li>
    <li class="breadcrumb-item active" aria-current="page"><a href="{{ route('dashboard.purchases.show',$invoiceData->id ) }}">{{ $invoiceData->reference_no }}</a></li>

@endcomponent

@include('backend.partials._errors')

@include('backend.purchases.payments.create')
                    ...
                    <tr>
                        ...
                        <td>
                              ...
                              <div class="modal fade" id="exampleModal{{ $row->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel{{ $row->id }}" aria-hidden="true">
                                ...
                                <form action="{{route('dashboard.purchases-payments.destroy', $row->id)}}" method="POST" style="display:inline-block">
                                    @csrf
                                 @method('delete')
      ...
      //  event.preventDefault();
       ...
       alert(paymentId);
        if(paymentId != null){
           ...
           $.ajax({
                url: "{{ url('dashboard/purchases-payments/payments/') }}"+'/'+paymentId,
                type:"GET",
