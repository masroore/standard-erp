...
<li class="file-tree-folder">{{ $lang == 'ar' ? $child_category->title_ar  : $child_category->title_en}}
    <a href="{{route('dashboard.accounts.edit', $child_category->id)}}"><i class="fa fa-edit"></i></a>
    <form action="{{route('dashboard.accounts.destroy', $child_category->id)}}" method="POST" style="display:inline-block">
        @csrf
        @method('delete')
